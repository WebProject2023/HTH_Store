...
/**
    ...
    public function check($proId, $cartId): array
   {
       $entity = $this->getEntityManager();
       return $entity->createQuery('
       SELECT COUNT(cd.id) as count, cd.quantity as quantity, cd.id as id FROM 
        App\Entity\CartDetail cd   
        where cd.cartid = :cartId AND cd.productid = :proId
       ')->setParameter('proId', $proId)
       ->setParameter('cartId', $cartId)
       ->getResult();
   }
